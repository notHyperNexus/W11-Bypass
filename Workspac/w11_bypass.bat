...
::# rebased on cmd due to defender transgression; skip already patched media (0b)

@echo off & title Bypass Windows 11 || Github 1.1.5.1908
...
::@echo off & echo;
%<%:f0 " W11 Bypass "%>>% & %<%:2f " Installed! Try run setup.exe "%>>% & %<%:f0 " run again to remove "%>%
...
::@echo off & echo;
%<%:f0 " W11 Bypass "%>>% & %<%:df " Removed! setup require you TPM "%>>% & %<%:f0 " run again to install "%>%
if /i "%CLI%"=="" timeout /t 7
exit /b 
